...
//                     {renderNews().map(item => (
//                         <NewsCard key={item.id} news={item} language={language} translations={uiTranslations[language]} />
//                     ))}
//                 </div>
//                 <div id="trending" className="trending" style={{ display: currentTab === 'trending' ? 'flex' : 'none' }}>
//                     {renderNews().map(item => (
//                         <NewsCard key={item.id} news={item} language={language} translations={uiTranslations[language]} />
//                     ))}
//                 </div>
//                 <div id="digest" className="digest" style={{ display: currentTab === 'digest' ? 'flex' : 'none' }}>
//                     {renderNews().map(item => (
//                         <NewsCard key={item.id} news={item} language={language} translations={uiTranslations[language]} />
//                     ))}
//                 </div>
//                 <div className="loading" style={{ display: page > 1 && currentTab === 'feed' ? 'block' : 'none' }}>
//                     {uiTranslations[language].modal.success.includes('Loading') ? 'Loading more stories...' : 'Loading more stories...'}
//                 </div>
//             </div>
//             <Modal
//                 isOpen={isModalOpen}
...
const uiTranslations = {
    ...
    // ... (keep other languages: hi, ta, te, bn, mr, kn, ml, gu, pa, or as provided)
...
function App() {
    ...
    const handleSearch = (e) => {
        ...
        setNews(query ? filtered : news); // Reset to full list if query is empty
        setCurrentTab('feed');
    };

    const filterNews = (cat) => {
        setCategory(cat);
        const filtered = cat === 'all' ? news : news.filter(item => item.category === cat);
        setNews(filtered);
        setIsSidebarOpen(false);
        setCurrentTab('feed');
    };
    ...
    const renderNews = () => {
        if (currentTab === 'trending') return news.slice(0, 1); // Show only the first item as trending
        if (currentTab === 'digest') return news;
        return news;
    };
    ...
    return (
        <div>
            ...
            <div className="container">
                ...
                <div id="feed" className="news-feed" style={{ display: currentTab === 'feed' ? 'flex' : 'none' }}>
                    {renderNews().map(item => (
                        <NewsCard key={item._id || item.id} news={item} language={language} translations={uiTranslations[language]} />
                    ))}
                </div>
                <div id="trending" className="trending" style={{ display: currentTab === 'trending' ? 'flex' : 'none' }}>
                    {renderNews().map(item => (
                ...
                <div className="loading" style={{ display: news.length === 0 ? 'block' : 'none' }}>
                    {uiTranslations[language].modal.success.includes('Loading') ? 'Loading...' : 'Loading...'}
                </div>
            </div>
            <Modal
                isOpen={isModalOpen}
